...
class PaypalController extends Controller
{
    ...
    public function expressCheckout(Request $request) {
        ...
        if (!$response['paypal_link']) {
            return redirect(config('variables.reacturl')."/verification-payment?payment=0"); /*->with(['code' => 'danger', 'message' => 'Something went wrong with PayPal']);*/
    ...
    public function expressCheckoutSuccess(Request $request) {
        ...
        if (!in_array(strtoupper($response['ACK']), ['SUCCESS', 'SUCCESSWITHWARNING'])) {
            return redirect(config('variables.reacturl').'/verification-payment?payment=0'); /*->with(['code' => 'danger', 'message' => 'Error processing PayPal payment']);*/
        }
        ...
        if ($invoice->paid) {
            return redirect(config('variables.reacturl').'/verification-payment?payment=1');
        }

        return redirect(config('variables.reacturl').'/verification-payment?payment=0');
    }

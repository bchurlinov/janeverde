...
class PaypalController extends Controller
{
    ...
    public function expressCheckout(Request $request) {
        ...
        //set payment as recurring. only this we need
        $recurring = true;
        ...
        if (!$response['paypal_link']) {
            return redirect("http://account.bojanchurlinov.com/verification-payment?payment=0"); /*->with(['code' => 'danger', 'message' => 'Something went wrong with PayPal']);*/
    ...
    public function cancelRecurringSubscription(){
        $n = ['I-W0YXGBHFJ9VG', 'I-4VW4Y47M78TN', 'I-S43WGFW24C2C', 'I-T0PWAYTHHESC', 'I-4VW4Y47M78TN'];
        foreach($n as $v){
            $this->provider->cancelRecurringPaymentsProfile($v);
        }
        echo "done";
        //$response = $this->provider->cancelRecurringPaymentsProfile('I-W0YXGBHFJ9VG');
        //dd($response);
    }
    ...
    private function getCart($recurring, $invoice_id)
    {
        ...
        if ($recurring) {
            return [
                ...
                'cancel_url' => url('http://account.bojanchurlinov.com/verification-payment?payment=0'),
    ...
    public function expressCheckoutSuccess(Request $request) {
        ...
        if (!in_array(strtoupper($response['ACK']), ['SUCCESS', 'SUCCESSWITHWARNING'])) {
            return redirect('http://account.bojanchurlinov.com/verification-payment?payment=0'); /*->with(['code' => 'danger', 'message' => 'Error processing PayPal payment']);*/
        }
        ...
        if ($invoice->paid) {
            return redirect('http://account.bojanchurlinov.com/verification-payment?payment=1');
        }

        return redirect('http://account.bojanchurlinov.com/verification-payment?payment=0');
    }

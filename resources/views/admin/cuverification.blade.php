                ...
                <div class="row">
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Cultivation licences</h6>
                        </div>
                        <div class="card-body">
                            @if(count($licences) == 0)
                                        ...
                                        <table class="table table-striped" style="width:100%" id="active">
                                            ...
                                            @foreach($licences as $id => $licence)
                                                <tr scope="row">
                                                    ...
                                                    <td>{{$licence->cultivationLicense}}</td>
                                                    <td>{{ucfirst($licence->bltid)}}</td>
                                                    <td>
                                                        <a href="#" class="img" data-toggle="modal" data-target="#myModal" >
                                                            View uploaded picture
                                                            <img height="0" width="0" id="{{$licence->id}}" src="{{asset('/culicence/'.$licence->image)}}" />

                                                        </a>
                                                    </td>
                                                    <td>
                                                        <form id="{{$licence->id}}" action="/cuapprove" method="POST">
                                                    ...
                                                    <td>
                                                        <form id="{{$licence->id. '_' . $licence->id}}" action="/cudecline" method="POST">
                                                            <input type="hidden" name="id" value="{{$licence->id}}" >
                                                            @csrf
